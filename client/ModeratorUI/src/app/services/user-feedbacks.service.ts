...
export class UserFeedbacksService {
  ...
  getUserFeedbacks(userId: number, pageNumber: number, pageSize: number)
    : Observable<ServiceResult<PaginatedList<UserFeedback>>> {
  ...
  deleteUserFeedback(commentId: number): Observable<ServiceMessage> {
    return this.httpClient.delete<ServiceMessage>(`${this.baseUrl}/feedbacks/${commentId}`);
  }
}
